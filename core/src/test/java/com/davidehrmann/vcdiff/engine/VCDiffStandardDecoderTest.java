...
public class VCDiffStandardDecoderTest extends VCDiffStandardDecoderTestBase {
    ...
    public void CopyInstructionsShouldFailIfNoSourceSegment() throws Exception {
        ...
        delta_window_header_ = VCDiffDecoderTest.ArraysExtra.replace(delta_window_header_, 0, 4, new byte[1]);
    ...
    public void TargetWindowSizeZero() throws Exception {
        byte[] zero_size = { 0x00 };
        delta_file_ = VCDiffDecoderTest.ArraysExtra.replace(delta_file_, delta_file_header_.length + 5, 2, zero_size);
    ...
    public void CopyMoreThanExpectedTarget() throws Exception {
        delta_file_[delta_file_header_.length + 0x70] =
                VCDiffDecoderTest.FirstByteOfStringLength(VCDiffDecoderTest.kExpectedTarget);
        delta_file_[delta_file_header_.length + 0x71] = (byte) (VCDiffDecoderTest.SecondByteOfStringLength(VCDiffDecoderTest.kExpectedTarget) + 1);
        decoder_.startDecoding(dictionary_);
        try {
            thrown.expect(IOException.class);
            decoder_.decodeChunk(delta_file_, output_);
    ...
    public void CopyAddressBeyondHereAddress() throws Exception {
        delta_file_[delta_file_header_.length + 0x7B] = VCDiffDecoderTest.FirstByteOfStringLength(VCDiffDecoderTest.kDictionary);
        delta_file_[delta_file_header_.length + 0x7C] = VCDiffDecoderTest.SecondByteOfStringLength(VCDiffDecoderTest.kDictionary);
        decoder_.startDecoding(dictionary_);
        try {
            thrown.expect(IOException.class);
            decoder_.decodeChunk(delta_file_, output_);
    ...
    public void AddMoreThanExpectedTarget() throws Exception {
        delta_file_[delta_file_header_.length + 0x72] =
                VCDiffDecoderTest.FirstByteOfStringLength(VCDiffDecoderTest.kExpectedTarget);
        delta_file_[delta_file_header_.length + 0x73] = (byte) (VCDiffDecoderTest.SecondByteOfStringLength(VCDiffDecoderTest.kExpectedTarget) + 1);
        decoder_.startDecoding(dictionary_);
        try {
            thrown.expect(IOException.class);
            decoder_.decodeChunk(delta_file_, output_);
    ...
    public void RunMoreThanExpectedTarget() throws Exception {
        delta_file_[delta_file_header_.length + 0x78] =
                VCDiffDecoderTest.FirstByteOfStringLength(VCDiffDecoderTest.kExpectedTarget);
        delta_file_[delta_file_header_.length + 0x79] = (byte) (VCDiffDecoderTest.SecondByteOfStringLength(VCDiffDecoderTest.kExpectedTarget) + 1);
        decoder_.startDecoding(dictionary_);
        try {
            thrown.expect(IOException.class);
            decoder_.decodeChunk(delta_file_, output_);
