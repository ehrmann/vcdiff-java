...
public class AppendableVCDiffStreamingEncoder extends BaseVCDiffStreamingEncoder<Appendable> {

    public AppendableVCDiffStreamingEncoder(HashedDictionary dictionary,
    ...
    private static CodeTableWriterInterface<Appendable> buildCodeTableWriter(EnumSet<VCDiffFormatExtensionFlag> format_extensions) {
        if (!format_extensions.contains(VCD_FORMAT_JSON)) {
            throw new IllegalArgumentException("format_extensions missing " + VCD_FORMAT_JSON);
        } else {
