...
public class RollingHashTest {
    ...
    public void TestModBase(long operand) {
        ...
        Assert.assertEquals((0xffffffffL & operand) % kBase, RollingHash.RollingHashUtil.ModBase(operand & 0xFFFFFFFFL));
        Assert.assertEquals((0x100000000L - ((0xFFFFFFFFL & operand) % kBase)) & 0xFFFFFFFFL, RollingHash.RollingHashUtil.FindModBaseInverse(operand));
        assertEquals(0, (int) (RollingHash.RollingHashUtil.ModBase(operand) + RollingHash.RollingHashUtil.FindModBaseInverse(operand)));
    }
    ...
    public void TestHashFirstTwoBytes(byte first_value, byte second_value) {
        ...
        Assert.assertEquals(RollingHash.RollingHashUtil.HashFirstTwoBytes(buf, 0), RollingHash.RollingHashUtil.HashStep(RollingHash.RollingHashUtil.HashStep(0, first_value), second_value));
        Assert.assertEquals(RollingHash.RollingHashUtil.HashFirstTwoBytes(buf, 0), RollingHash.RollingHashUtil.HashStep(first_value & 0xff, second_value));
    }
